...
def pkt_parser(pkt):
    ...
    if pkt.haslayer(UDP) and pkt.haslayer(IP) and pkt.haslayer(Raw):
        ...
        if kerb_hash:
            print 'kerb_hash'
            printer(pkt.time,src_ip_port, dst_ip_port, kerb_hash)
        ...
        if 0 < len(full_load) < 750:
            ...
            if len(ftp_creds) > 0:
                ...
                for msg in ftp_creds:
                    printer(pkt.time,src_ip_port, dst_ip_port, msg)
                return
            ...
            if irc_creds != None:
                print 'irc_creds'
                printer(pkt.time,src_ip_port, dst_ip_port, irc_creds)
                return
    ...
    # seq always = last ack of tcp stream
        ...
        if seq in mail_auths[dst_ip_port][-1]:
            ...
            if full_load.startswith('235') and 'auth' in full_load.lower():
                ...
                printer(pkt.time,dst_ip_port, src_ip_port, a_s)
                found = True
                try:
            ...
            # SMTP failed
                ...
                # Reversed the dst and src
                printer(pkt.time,dst_ip_port, src_ip_port, a_f)
                found = True
                try:
                    del mail_auths[dst_ip_port]
                except KeyError:
...
def other_parser(src_ip_port, dst_ip_port, full_load, ack, seq, pkt, verbose):
    ...
    if http_line != None:
        ...
        if http_url_req != None:
            ...
            printer(pkt.time,src_ip_port, None, http_url_req)

    # Print search terms
    searched = get_http_searches(http_url_req, body, host)
    if searched:
        printer(pkt.time,src_ip_port, dst_ip_port, searched)
    ...
    if body != '':
        ...
        if user_passwd != None:
            try:
                ...
                printer(pkt.time,src_ip_port, dst_ip_port, user_msg)
                pass_msg = 'HTTP password: %s' % http_pass
                printer(pkt.time,src_ip_port, dst_ip_port, pass_msg)
            except UnicodeDecodeError:
                pass
    ...
    if method == 'POST' and 'ocsp.' not in host:
        try:
            ...
            printer(pkt.time,src_ip_port, None, msg)
        except UnicodeDecodeError:
            pass
    ...
    if NTLMSSP3:
        ...
        if ntlm_resp_found != None:
            printer(pkt.time,src_ip_port, dst_ip_port, ntlm_resp_found)
    ...
    if authorization_header or authenticate_header:
        ...
        if netntlm_found != None:
            printer(pkt.time,src_ip_port, dst_ip_port, netntlm_found)
...
def printer(pkgtime,src_ip_port, dst_ip_port, msg):
    if dst_ip_port != None:
        print_str = '[%s][%s][%s > %s] %s%s%s' % (pkgtime,str(arrow.get(pkgtime)),src_ip_port, dst_ip_port, T, msg, W)
        ...
        print_str = '[%s][%s][%s] %s' % (pkgtime,str(arrow.get(pkgtime)),src_ip_port.split(':')[0], msg)
        print print_str
